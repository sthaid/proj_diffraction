...
static int interferometer_diagram_pane_hndlr(pane_cx_t * pane_cx, int request, void * init_params, sdl_event_t * event)
{
   ...
   #define SDL_EVENT_ZOOM     (SDL_EVENT_USER_DEFINED + 0)
    ...
    if (request == PANE_HANDLER_REQ_RENDER) {
        param_t *p = &param[interference_pattern_idx];
   ...
   #define SDL_EVENT_PARAM_SELECT (SDL_EVENT_USER_DEFINED + 0)
    ...
    if (request == PANE_HANDLER_REQ_RENDER) {
        ...
        for (i = 0; i < max_param; i++) {
            sdl_render_text_and_register_event(
                ...
                param[i].name,
                (i != interference_pattern_idx ? LIGHT_BLUE : WHITE), BLACK,
    ...
    if (request == PANE_HANDLER_REQ_EVENT) {
        ...
        case SDL_EVENT_PARAM_SELECT...SDL_EVENT_PARAM_SELECT+MAX_PARAM-1:
            ...
            interference_pattern_idx = event->event_id - SDL_EVENT_PARAM_SELECT;
            calculate_interferometer_diagram(&param[interference_pattern_idx]);
            DEBUG("set interference_pattern_idx = %d\n", interference_pattern_idx);
            break;
        }
        return PANE_HANDLER_RET_DISPLAY_REDRAW;
    }

XXX  REFER TO RESULTS DIR
XXX  results dir name
...
The data is stored in a file so that a graphing program, such as
gnuplot, can be later used to plot the graph of count rate vs. 
sensor position.

There are 2 programs, and I run each program on a seperate Raspberry Pi. 
...
run on a dedicated Raspberry Pi Model 4 XXX.
...
and requests SiPM count rate from the sipm_server for each position 
of the SiPM sensor.

The position/count_rate data is stored to a file, so it can later
be plotted; and a crude plot of the data is also displaed in real time
on the terminal.
...
factor of 10^7.6. The reason for this is to usually have just one photon at a time
in the region between the Filters and the SiPM detector. 
...
However an interference pattern does occur, even with a single photon. Quanum Mechanics 
...
Translation Rail Stepper Motor Controller
...
Power Supply for Translation Rail
...
Total Cost of Parts Listed Above         xxxxxxxxxx
...
This is implemented in the sipm_thread:
    while (true) {
        read_sipm();
        analyze_sipm();
    }
...
- to calibrate the xrail
  ...
  - c:  calibrate done
- commands valid when calibrated
  - h:  goto home
  - g:  position xrail in range -10mm to +10mm in 0.1mm steps, and collect SiPM rate;
...
- + -:    adjust y span
- HOME:   position graph at begining
- END:    position graph at the end
- r:      reset y offset and y span

Usage Example:
- ./ctlr:  starts the program
- F7 / F8: to position the xrail to home (center) position
...
- This can be done on any Linux computer which has gnuplot available.
  To get gnuplot, on Fedora, 'yum install gnuplot'.
- Refer to results/run_gnuplot shell script.

Debugging:
- Check the ctlr.log file.
- Ensure festival text to speech server is running:
